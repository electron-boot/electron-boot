...
import { getProviderId, getProviderName, getProviderUUId } from '../decorators/decorator.manager'

const PREFIX = '_id_default_'

class LegacyIdentifierRelation extends Map<ObjectIdentifier, string> implements IIdentifierRelationShip {
  saveClassRelation(module: any, namespace?: string) {
    const providerId = getProviderUUId(module)
    // save uuid
    this.set(providerId, typeof providerId === 'undefined' ? '' : providerId.toString())
    if (providerId) {
      // save alias id
      ...
      if (namespace) {
        this.set(namespace + ':' + (typeof providerName === 'undefined' ? '' : providerName.toString()), providerId)
      }
    }
  }
...
export class ObjectDefinitionRegistry extends Map implements IObjectDefinitionRegistry {
  ...
  registerObject(identifier: ObjectIdentifier, target: any): void {
    this.set(PREFIX + (identifier as string), target)
  }
