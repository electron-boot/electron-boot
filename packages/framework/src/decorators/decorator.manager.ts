...
export const IPC_KEY = 'common:ipc'
...
export class DecoratorManager extends Map implements IModuleStore {
   ...
   * @param propertyName
   */
  saveMetadata(decoratorNameKey: ObjectIdentifier, data: any, target: any, propertyName?: string | symbol): void {
    if (propertyName) {
   ...
   * @param propertyName
   */
  getMetadata(decoratorNameKey: ObjectIdentifier, target: any, propertyName?: string | symbol): any {
    if (propertyName) {
      const dataKey = DecoratorManager.getDecoratorMethod(decoratorNameKey, propertyName)
   ...
   * @param propertyName
   */
  getPropertyDataFromClass(decoratorNameKey: ObjectIdentifier, target: any, propertyName: string | symbol): any {
    const dataKey = DecoratorManager.getDecoratorClsMethodKey(decoratorNameKey, propertyName)
...
if (typeof global === 'object') {
  if (global['ELECTRON_APPLICATION_DECORATOR_MANAGER']) {
    ...
    const coreModulePathList = getModuleRequirePathList('@electron-boot/framework')
 ...
 * @param target
 */
export function getClassMetadata<T = any>(decoratorNameKey: ObjectIdentifier, target: any): T {
 ...
 * @param target
 */
export function listPropertyDataFromClass(decoratorNameKey: ObjectIdentifier, target: any): any[] {
 ...
 * @param propertyName
 */
export function getPropertyMetadata<T = any>(decoratorNameKey: ObjectIdentifier, target: any, propertyName: any): T {
 ...
 * @param target
 */
export function saveModule(decoratorNameKey: ObjectIdentifier, target: any): void {
  if (isClass(target)) {
 ...
 * @param filter
 */
export function listModule(decoratorNameKey: ObjectIdentifier, filter?: (module) => boolean): any[] {
...
export function getClassExtendedMetadata<T = any>(
  ...
  if (father && father.constructor !== Object) {
    metadata = merge(
      getClassExtendedMetadata(decoratorNameKey, father, propertyName, useCache),
 ...
 */
export function getMethodParamTypes(target: any, methodName: string | symbol): any {
  if (isClass(target)) {
 ...
 * @param methodName
 */
export function getPropertyType(target: any, methodName: string | symbol): TSDesignType {
 ...
 */
export function createCustomPropertyDecorator(decoratorKey: string, metadata: any, impl = true): PropertyDecorator {
...
export function createCustomMethodDecorator(
  ...
  return function (target: any, propertyName: string | symbol, _descriptor: TypedPropertyDescriptor<any>) {
    attachClassMetadata(
