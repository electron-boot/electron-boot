...
const CONFIG_FILE_NAME = 'electron.boot.config';
...
async function bundleConfigFile(fileName: string, isESM: boolean): Promise<{ code: string; dependencies: string[] }> {
  const dirnameVarName = '__electron_boot_injected_dirname';
  const filenameVarName = '__electron_boot_injected_filename';
  const importMetaUrlVarName = '__electron_boot_injected_import_meta_url';
