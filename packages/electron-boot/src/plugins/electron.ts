  ...
  return [
    {
      ...
      config(config: ViteConfig): void {
        ...
        const nodeTarget: string = getElectronNodeTarget();

        const pkg: PackageData = loadPackageData() || { type: 'commonjs' };

        const format: LibraryFormats = pkg.type && pkg.type === 'module' && supportESM() ? 'es' : 'cjs';
        ...
        if (!rollupOptions.input) {
          ...
          (defaultConfig.build!.rollupOptions!.output! as OutputOptions).format = format;
        }

        (defaultConfig.build!.rollupOptions!.output as OutputOptions)['assetFileNames'] = path.posix.join(
    ...
    },
    {
      ...
      configResolved(config): void {
        ...
        if (!build.target) {
          ...
          const targets = Array.isArray(build.target) ? build.target : [build.target];
          if (targets.some((t) => !t.startsWith('chrome') && !/^es(?:202\d|next)$/.test(t))) {
            config.logger.warn(
