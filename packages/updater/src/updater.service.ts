...
import type { AppUpdaterEvent, BootUpdateDownloaded, ElectronUpdaterEvent, UpdaterConfig } from './types'
...
export class UpdaterService extends (EventEmitter as new () => TypedEmitter<AppUpdaterEvent>) {
  ...
  constructor(_ctx: GenericApplicationContext, config: UpdaterConfig) {
    ...
    // 如果配置了allowDowngrade则使用allowDowngrade，没有配置则不管
    if (config.allowDowngrade) autoUpdater.allowDowngrade = config.allowDowngrade
    ;[
      'error',
      'checking-for-update',
      ...
      autoUpdater.on(value as keyof ElectronUpdaterEvent, function () {
        ...
        if (value === 'update-not-available') {
          ...
          if (isEmpty(platform)) throw new Error(`this platform ${process.platform} can not use hot updater`)
          try {
            axios
              .get<string, AxiosResponse<string>>(path.join(appUpdaterConfig.url, `hot-latest-${platform}.yml`), {
                responseType: 'text',
                headers: {
                ...
                if (remoteVersion != localVersion) {
                  ...
                  axios
                    ...
                    .then(res => {
                      ...
                      fs.writeFile(path.join(app.getPath('userData'), filename), res.data, err => {
                        if (err) {
